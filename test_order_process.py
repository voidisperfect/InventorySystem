...
"""
    ...
    response = httpx.post(f"{ORDER_URL}/token", data={"username": "testuser", "password": "password"})
    ...
    response = httpx.post(f"{ORDER_URL}/api/v1/orders/", json=order_data, headers=headers)
    ...
    assert float(actual_price) == expected_price, f"Expected {expected_price}, got {actual_price}"
